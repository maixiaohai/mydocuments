...
;;  YOU WRITE THIS PART:
(defclass dice-set ()
  ((value-list :initform nil)) ;; WRITE DICE-SET CLASS BODY HERE
)

(defmethod get-values ((object dice-set))
  ;; WRITE GET-VALUES METHOD DEFINITION HERE
  (slot-value object 'value-list)
)

(defmethod roll (how-many (object dice-set))
  ;; WRITE ROLL METHOD DEFINITION HERE
  (setf (slot-value object 'value-list) nil)
  (dotimes (x how-many)
    (push (1+ (random 6)) (slot-value object 'value-list)))
  (slot-value object 'value-list)
)
