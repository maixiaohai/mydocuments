...
(defun triangle (a b c)
  (let ((judgement (loop for x in (list a b c)
                      collect x into collected
                      maximize x into maxed
                      minimize x into mined
                      finally (return (list mined (cadr (sort collected #'<)) maxed)))))
    (cond ((or (< (first judgement) 1)
               (<= (+ (first judgement)
                      (second judgement))
                   (third judgement)))
